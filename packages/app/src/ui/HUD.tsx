...
function ScoreBar({ state }: { state: GameState }) {
  return (
    <div className="scorebar">
      ...
      <div className="activecolor" style={{ background: PALETTE[state.activeColor] }} title={t(`color.${state.activeColor}`)} />
    </div>
  );
}

function TurnBanner({ state }: { state: GameState }) {
  if (state.phase !== 'playing' && state.phase !== 'choosing_color' && state.phase !== 'challenge_window') return null;
  const mine = state.currentPlayer === HUMAN_ID;
  return <div className={`turnbanner${mine ? ' mine' : ''}`}>{mine ? t('hud.yourTurn') : t('hud.turnOf', { name: playerName(state, state.currentPlayer) })}</div>;
}
  ...
  return (
    <div className="actionbar">
      {myTurn && state.drawnCard === undefined && (
        <button className="btn" onClick={() => dispatch({ type: 'DRAW_CARD', player: HUMAN_ID })}>{t('hud.draw')}</button>
      )}
      {myTurn && state.drawnCard !== undefined && (
      ...
      {state.unoVulnerable && state.unoVulnerable !== HUMAN_ID && (
        <button className="btn catch" onClick={() => dispatch({ type: 'CATCH_UNO', player: HUMAN_ID, target: state.unoVulnerable! })}>
  ...
  return (
    <div className="modal">
      ...
      <div className="colors">
        {COLORS.map((c) => (
          <button key={c} className="swatch" style={{ background: PALETTE[c] }} aria-label={t(`color.${c}`)}
            onClick={() => dispatch({ type: 'CHOOSE_COLOR', player: HUMAN_ID, color: c })} />
        ))}
      </div>
    </div>
  );
...
function ChallengePrompt({ state }: { state: GameState }) {
  const dispatch = useGameStore((s) => s.dispatch);
  if (state.phase !== 'challenge_window' || state.draw4Challenge?.target !== HUMAN_ID) return null;
  return (
    <div className="modal">
      <h2>{t('hud.challengePrompt', { name: playerName(state, state.draw4Challenge.player) })}</h2>
      <div className="row">
        <button className="btn" onClick={() => dispatch({ type: 'ACCEPT_DRAW4', player: HUMAN_ID })}>{t('hud.accept')}</button>
        <button className="btn primary" onClick={() => dispatch({ type: 'CHALLENGE_DRAW4', player: HUMAN_ID })}>{t('hud.challenge')}</button>
      </div>
    </div>
  );
}
...
function RoundOver({ state, onNewGame }: { state: GameState; onNewGame: () => void }) {
  ...
  return (
    <div className="modal">
      ...
      <p>{gameOver ? t('hud.gameWinner', { name: playerName(state, winner) }) : t('hud.roundWinner', { name: playerName(state, winner), points: state.players.find((p) => p.id === winner)!.score })}</p>
      <table className="scores">
        <tbody>
          {state.players.map((p) => (
            <tr key={p.id}><td>{playerName(state, p.id)}</td><td>{p.score}</td></tr>
          ))}
        </tbody>
      </table>
      {gameOver
        ? <button className="btn primary" onClick={onNewGame}>{t('hud.newGame')}</button>
        : <button className="btn primary" onClick={() => dispatch({ type: 'START_ROUND' })}>{t('hud.nextRound')}</button>}
    </div>
  );
}
