    ...
    <div className="lobby">
      ...
      <div className="row">
        {OPPONENT_OPTIONS.map((n) => (
          <button key={n} className={`chip${settings.opponents === n ? ' on' : ''}`} onClick={() => onChange({ ...settings, opponents: n })}>{n}</button>
        ))}
      </div>
      <label>{t('lobby.difficulty')}</label>
      <div className="row">
        {DIFFICULTIES.map((d) => (
          <button key={d} className={`chip${settings.difficulty === d ? ' on' : ''}`} onClick={() => onChange({ ...settings, difficulty: d })}>{t(`difficulty.${d}`)}</button>
        ))}
      </div>
      <button className="btn primary big" onClick={onPlay}>{t('lobby.play')}</button>
    </div>
  );
}
