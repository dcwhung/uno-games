...
function label(face: CardFace): { big: string; font: string } {
  switch (face.kind) {
    case 'number': return { big: String(face.value ?? ''), font: BIG_FONT };
    case 'skip': return { big: '⊘', font: BIG_FONT };
    case 'reverse': return { big: '⇄', font: BIG_FONT };
    case 'draw2': return { big: '+2', font: ACTION_FONT };
    case 'wild': return { big: 'W', font: ACTION_FONT };
    case 'wild_draw4': return { big: '+4', font: ACTION_FONT };
    default: return { big: face.kind, font: SMALL_FONT };
  }
}
...
export function faceTexture(face: CardFace): THREE.CanvasTexture {
  ...
  ctx.fill();
  ctx.fillStyle = bg;
  roundedRect(ctx, INNER_INSET, INNER_INSET, TEX_W - 2 * INNER_INSET, TEX_H - 2 * INNER_INSET, CORNER - 6);
  ctx.fill();
