...
function Opponent({ seat, color, active, catchable, name, cards, calledUno, onCatch }: OpponentProps) {
  const yaw = Math.atan2(seat[0], seat[2]) + Math.PI;
  return (
    <group position={[seat[0], seat[1], seat[2]]} rotation={[0, yaw, 0]} onClick={() => { if (catchable) onCatch(); }}>
...
function TableContents({ state }: { state: GameState }) {
  ...
  const targets = useMemo(() => computeLayout(state, HUMAN_ID, selected, legal), [state, selected, legal]);
    ...
    <>
      ...
      {opponents.map((p, i) => (
        <Opponent
          ...
          active={state.currentPlayer === p.id && state.phase !== 'round_over' && state.phase !== 'game_over'}
...
export function Scene() {
  ...
  return (
    <Canvas shadows dpr={[1, 2]} camera={{ position: [...CAMERA_POS], fov: FOV_PORTRAIT, near: 0.1, far: 30 }}
      ...
      <directionalLight position={[2, 6, 3]} intensity={1.4} castShadow shadow-mapSize={[1024, 1024]} />
