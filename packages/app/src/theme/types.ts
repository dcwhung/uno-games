...
export type SingleSidedTheme = CardTheme & {
    readonly sides: { front: SideTheme; back: undefined };
};
export type DoubleSidedTheme = CardTheme & {
    readonly sides: { front: SideTheme; back: SideTheme };
};
...
export function cellToUv(
    grid: Pick<AtlasGrid, 'columns' | 'rows' | 'paddingPx' | 'texture'>,
    cell: number,
): UvRect {
