...
function simulate(difficulty: BotDifficulty, seed: number, n = 4): { state: GameState; steps: number; actions: Action[] } {
  const bots = Object.fromEntries(Array.from({ length: n }, (_, i) => [P(i), createBot(difficulty)]));
  let rng = createRng(seed);
  let s = engine.createInitialState({ ...CONFIG, targetScore: 200 }, seed);
  const actions: Action[] = [{ type: 'START_GAME', players: players(n) }, { type: 'START_ROUND' }];
  ...
  while (s.phase !== 'game_over' && steps < MAX_STEPS) {
    ...
    const catcher = s.unoVulnerable ? s.players.find((p) => p.id !== s.unoVulnerable)!.id : undefined;
    ...
    if (rejected) throw new Error(`Bot ${who} action rejected: ${JSON.stringify(rejected)} in phase ${s.phase}`);
    s = r.state;
  }
...
describe('bots', () => {
  ...
  it('card conservation holds after every step', () => {
    ...
    for (const a of actions) {
      ...
      expect(inHands + s.drawPile.length + s.discardPile.length).toBe(Object.keys(s.cards).length);
    }
  });
  ...
  it('medium bot chooses its most common colour for a wild', () => {
    ...
    for (const c of view.myHand) if (c.front.color !== 'wild') counts[c.front.color] = (counts[c.front.color] ?? 0) + 1;
