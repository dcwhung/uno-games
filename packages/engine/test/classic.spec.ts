...
describe('classic deck', () => {
  it('has 108 cards with the official composition', () => {
    ...
    const count = (pred: (f: (typeof deck)[number]['front']) => boolean) => deck.filter((c) => pred(c.front)).length;
...
describe('start of round', () => {
  ...
  it('rejects 1 or 5 players', () => {
    ...
    expect(types(engine.apply(s, { type: 'START_GAME', players: players(1) }))).toEqual(['ActionRejected']);
    expect(types(engine.apply(s, { type: 'START_GAME', players: players(5) }))).toEqual(['ActionRejected']);
  });
...
describe('legal moves', () => {
  ...
  it('matches action kind across colours', () => {
    const s = rig(base, {
      ...
      hands: { [P(0)]: [{ color: 'blue', kind: 'skip' }, { color: 'blue', kind: 'reverse' }] },
    });
  ...
  it('uses activeColor, not the wild top card colour', () => {
    const s = rig(base, {
      ...
      activeColor: 'green',
      hands: { [P(0)]: [{ color: 'green', kind: 'number', value: 1 }, { color: 'red', kind: 'number', value: 1 }] },
    });
    expect(engine.getLegalMoves(s, P(0)).map((m) => faceOf(s, m.card).color)).toEqual(['green']);
  });

  it('returns nothing when it is not your turn', () => {
    const s = rig(base, { top: { color: 'red', kind: 'number', value: 1 }, hands: { [P(1)]: [{ color: 'red', kind: 'number', value: 2 }] } });
  ...
  it('rejects an illegal play and a card not in hand', () => {
    const s = rig(base, {
      ...
      hands: { [P(0)]: [{ color: 'blue', kind: 'number', value: 2 }], [P(1)]: [{ color: 'red', kind: 'number', value: 3 }] },
    });
    expect(play(s, P(0), firstCard(s, P(0))).events[0]).toMatchObject({ type: 'ActionRejected', reason: 'illegal_card' });
    expect(play(s, P(0), firstCard(s, P(1))).events[0]).toMatchObject({ type: 'ActionRejected', reason: 'card_not_in_hand' });
    expect(play(s, P(1), firstCard(s, P(1))).events[0]).toMatchObject({ type: 'ActionRejected', reason: 'not_your_turn' });
  });
});
...
describe('action cards', () => {
  ...
  const filler = (color: 'red' | 'blue' = 'blue') => [{ color, kind: 'number' as const, value: 3 }, { color, kind: 'number' as const, value: 4 }];

  it('number card advances to the next player', () => {
    const s = rig(base, { top: { color: 'red', kind: 'number', value: 1 }, hands: { [P(0)]: [{ color: 'red', kind: 'number', value: 2 }, ...filler()] } });
  ...
  it('skip jumps over the next player', () => {
    const s = rig(base, { top: { color: 'red', kind: 'number', value: 1 }, hands: { [P(0)]: [{ color: 'red', kind: 'skip' }, ...filler()] } });
    const r = play(s, P(0), firstCard(s, P(0)));
  ...
  it('reverse flips direction with 3+ players', () => {
    const s = rig(base, { top: { color: 'red', kind: 'number', value: 1 }, hands: { [P(0)]: [{ color: 'red', kind: 'reverse' }, ...filler()] } });
    const r = play(s, P(0), firstCard(s, P(0)));
  ...
  it('reverse acts as skip with 2 players', () => {
    ...
    const s = rig(two, { top: { color: 'red', kind: 'number', value: 1 }, hands: { [P(0)]: [{ color: 'red', kind: 'reverse' }, ...filler()] } });
    const r = play(s, P(0), firstCard(s, P(0)));
  ...
  it('draw two makes the next player draw 2 and lose their turn', () => {
    const s = rig(base, { top: { color: 'red', kind: 'number', value: 1 }, hands: { [P(0)]: [{ color: 'red', kind: 'draw2' }, ...filler()], [P(1)]: filler() } });
    const r = play(s, P(0), firstCard(s, P(0)));
    expect(hand(r.state, P(1))).toHaveLength(4);
    expect(r.events).toContainEqual(expect.objectContaining({ type: 'CardDrawn', player: P(1), reason: 'draw2' }));
    expect(r.state.currentPlayer).toBe(P(2));
  ...
  it('wild without a colour waits for CHOOSE_COLOR, then advances', () => {
    const s = rig(base, { top: { color: 'red', kind: 'number', value: 1 }, hands: { [P(0)]: [{ color: 'wild', kind: 'wild' }, ...filler()] } });
    const r = play(s, P(0), firstCard(s, P(0)));
...
describe('wild draw four', () => {
  ...
  it('opens a challenge window for the next player', () => {
    ...
    expect(r.state.draw4Challenge).toMatchObject({ player: P(0), target: P(1), priorColor: 'red', wasBluff: false });
    expect(engine.apply(r.state, { type: 'ACCEPT_DRAW4', player: P(2) }).events[0]).toMatchObject({ type: 'ActionRejected' });
  });
  ...
  it('successful challenge: bluffer draws 4, challenger plays next', () => {
    ...
    expect(c.events).toContainEqual({ type: 'Draw4Challenged', by: P(1), against: P(0), succeeded: true });
  ...
  it('failed challenge: challenger draws 6 and is skipped', () => {
    ...
    const r = play(s, P(0), firstCard(s, P(0)), 'blue');
    const c = engine.apply(r.state, { type: 'CHALLENGE_DRAW4', player: P(1) });
    expect(c.events).toContainEqual({ type: 'Draw4Challenged', by: P(1), against: P(0), succeeded: false });
  ...
  it('wild draw four as the last card still ends the round after the penalty', () => {
    const s = rig(base, {
      top: { color: 'red', kind: 'number', value: 1 },
      hands: { [P(0)]: [{ color: 'wild', kind: 'wild_draw4' }], [P(1)]: [{ color: 'green', kind: 'number', value: 2 }], [P(2)]: [{ color: 'blue', kind: 'skip' }] },
    });
    const r = play(s, P(0), firstCard(s, P(0)), 'blue');
    const a = engine.apply(r.state, { type: 'ACCEPT_DRAW4', player: P(1) });
...
describe('drawing', () => {
  ...
  it('unplayable drawn card ends the turn immediately', () => {
    const s = rig(base, { top: { color: 'red', kind: 'number', value: 1 }, hands: { [P(0)]: [{ color: 'blue', kind: 'number', value: 9 }] } });
    // Force the top of the draw pile to be unplayable.
    const unplayable = s.drawPile.find((id) => faceOf(s, id).color === 'green' && faceOf(s, id).kind === 'number' && faceOf(s, id).value !== 1)!;
    const rigged: GameState = { ...s, drawPile: [...s.drawPile.filter((id) => id !== unplayable), unplayable] };
  ...
  it('playable drawn card may be played or passed, but no other card', () => {
    const s = rig(base, { top: { color: 'red', kind: 'number', value: 1 }, hands: { [P(0)]: [{ color: 'red', kind: 'number', value: 9 }] } });
    const playable = s.drawPile.find((id) => faceOf(s, id).color === 'red' && faceOf(s, id).kind === 'number' && faceOf(s, id).value === 5)!;
    const rigged: GameState = { ...s, drawPile: [...s.drawPile.filter((id) => id !== playable), playable] };
    const r = engine.apply(rigged, { type: 'DRAW_CARD', player: P(0) });
    ...
    expect(engine.apply(r.state, { type: 'DRAW_CARD', player: P(0) }).events[0]).toMatchObject({ type: 'ActionRejected' });
  ...
  it('reshuffles the discard pile (minus top) when the draw pile is empty', () => {
    const s = rig(base, { top: { color: 'red', kind: 'number', value: 1 }, hands: { [P(0)]: [{ color: 'blue', kind: 'number', value: 9 }] }, drawPileSize: 0 });
    const discards = s.drawPile; // empty
    expect(discards).toHaveLength(0);
    const spare = (Object.keys(s.cards) as CardId[]).filter((id) => !hand(s, P(0)).includes(id) && id !== s.discardPile[0]).slice(0, 10);
...
describe('UNO', () => {
  ...
  const twoCards = () =>
    rig(base, {
      top: { color: 'red', kind: 'number', value: 1 },
      hands: { [P(0)]: [{ color: 'red', kind: 'number', value: 2 }, { color: 'blue', kind: 'number', value: 3 }], [P(1)]: [{ color: 'red', kind: 'number', value: 4 }, { color: 'green', kind: 'number', value: 4 }, { color: 'green', kind: 'number', value: 6 }] },
    });
  ...
  it('calling UNO before playing protects you', () => {
    ...
    expect(engine.apply(r.state, { type: 'CATCH_UNO', player: P(1), target: P(0) }).events[0]).toMatchObject({ type: 'ActionRejected' });
  });
  ...
  it('calling UNO just after playing also works, and cannot be called twice', () => {
    ...
    expect(engine.apply(u.state, { type: 'CALL_UNO', player: P(0) }).events[0]).toMatchObject({ reason: 'already_called' });
  });

  it('cannot call UNO with 3+ cards, and you cannot catch yourself', () => {
    const s = rig(base, { top: { color: 'red', kind: 'number', value: 1 }, hands: { [P(0)]: [{ color: 'red', kind: 'number', value: 2 }, { color: 'blue', kind: 'number', value: 3 }, { color: 'blue', kind: 'number', value: 5 }] } });
    expect(engine.apply(s, { type: 'CALL_UNO', player: P(0) }).events[0]).toMatchObject({ type: 'ActionRejected' });
    const two = twoCards();
    const r = play(two, P(0), firstCard(two, P(0)));
    expect(engine.apply(r.state, { type: 'CATCH_UNO', player: P(0), target: P(0) }).events[0]).toMatchObject({ reason: 'no_uno_to_catch' });
  });
  ...
  it('the window closes once the next player acts', () => {
    ...
    expect(engine.apply(n.state, { type: 'CATCH_UNO', player: P(2), target: P(0) }).events[0]).toMatchObject({ type: 'ActionRejected' });
  });
...
describe('scoring', () => {
  ...
  it('winner collects face value, 20 per action card, 50 per wild', () => {
    const s = rig(base, {
      ...
      hands: {
        ...
        [P(1)]: [{ color: 'blue', kind: 'number', value: 7 }, { color: 'green', kind: 'skip' }],
        [P(2)]: [{ color: 'wild', kind: 'wild' }, { color: 'wild', kind: 'wild_draw4' }, { color: 'red', kind: 'number', value: 0 }],
      },
    });
    const r = play(s, P(0), firstCard(s, P(0)));
    expect(r.state.phase).toBe('round_over');
    expect(r.events).toContainEqual(expect.objectContaining({ type: 'RoundEnded', winner: P(0), points: 7 + 20 + 50 + 50 + 0 }));
  ...
  it('ends the game at the target score and rejects further rounds', () => {
    ...
    const s = rig(low, {
      top: { color: 'red', kind: 'number', value: 1 },
      hands: { [P(0)]: [{ color: 'red', kind: 'number', value: 2 }], [P(1)]: [{ color: 'wild', kind: 'wild' }, { color: 'wild', kind: 'wild' }] },
    });
    const r = play(s, P(0), firstCard(s, P(0)));
    expect(r.state.phase).toBe('game_over');
    expect(r.state.gameWinner).toBe(P(0));
    expect(types(r)).toContain('GameEnded');
    expect(engine.apply(r.state, { type: 'START_ROUND' }).events[0]).toMatchObject({ type: 'ActionRejected' });
  });

  it('next round rotates the dealer and keeps scores', () => {
    const s = rig(base, { top: { color: 'red', kind: 'number', value: 1 }, hands: { [P(0)]: [{ color: 'red', kind: 'number', value: 2 }], [P(1)]: [{ color: 'blue', kind: 'number', value: 9 }] } });
    const r = play(s, P(0), firstCard(s, P(0)));
...
describe('replay', () => {
  it('rebuilds identical state from seed + actions', () => {
    const actions: Action[] = [{ type: 'START_GAME', players: players(3) }, { type: 'START_ROUND' }];
    ...
    for (let i = 0; i < 6 && s.phase === 'playing'; i++) {
      ...
      const a: Action = legal[0]
        ? { type: 'PLAY_CARD', player: s.currentPlayer, card: legal[0].card, chosenColor: 'blue' }
...
describe('opening card', () => {
  ...
  it('VARIANT actions are rejected by Classic', () => {
    ...
    expect(engine.apply(state, { type: 'VARIANT', player: P(0), payload: null }).events[0]).toMatchObject({ reason: 'variant_rule' });
  });
});
